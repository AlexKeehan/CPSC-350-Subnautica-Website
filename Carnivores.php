...
<head>
    ...
    <ul class="nav-links">
        ...
        <li><a class="active" href="Carnivores.php">Carnivores</a></li>
...
<form action="Carnivores.php" method="post">
    <div>Search Type: <input type="text" name="type" placeholder="Ex: Name, Id, etc ..."></div>
    <div>Search: <input type="text" name="search" placeholder="Ex: Ampeel, 2, etc ..."></div>
    <?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (!empty($type)) {
            ...
            if (!empty($search) || $search == 0) {
                if ($type == "Biome") {
                    $sql = "select Specie_id, Type_id, Specie_Name, Biome_Name, Type_Name, Specie_Dangerous, Specie_Dang_Lvl, Specie_Bio from Species natural join Types natural join Biomes_Species natural join Biomes where Type_id=1 and Biome_id='$search' order by Specie_id";
                }else if ($type == "Specie_id" || $type == "Type_id" || $type == "Specie_Dangerous" || $type == "Specie_Dang_Lvl") {
                    $sql = "Select Specie_id, Type_id, Specie_Name, Biome_Name, Type_Name, Specie_Dangerous, Specie_Dang_Lvl, Specie_Bio from Species natural join Types natural join Biomes_Species natural join Biomes where Type_id=1 and " . $type . " = $search order by Specie_id";
                }else if ($type == "Specie_Name" || $type ==  "Specie_Bio") {
                    $sql = "Select Specie_id, Type_id, Specie_Name, Biome_Name, Type_Name, Specie_Dangerous, Specie_Dang_Lvl, Specie_Bio from Species natural join Types natural join Biomes_Species natural join Biomes where Type_id=1 and " . $type . " like '%$search%' order by Specie_id";
                }else {
                    header("Refresh:1");
                }
                ...
                if ($num_results == 0) {
                    header('Refresh:1;URL=Carnivores.php');
                }
            }
            else if (!empty($edit)) {
                if ($type == "Biome") {
                ...
                $sql = "insert into Species(Specie_Name, Type_id, Specie_Dangerous, Specie_Dang_Lvl, Specie_Bio)values('$add_name', 1,$add_dangerous,$add_dang_lvl,'$add_bio')";
                $result = $connection->query($sql);
...
<?php
        ...
        if (empty($type) && empty($search) && empty($edit) && empty($add_name) && empty($delete)) {
            $sql = "Select Specie_id, Type_Name, Specie_Name, Biome_Name, Specie_Dangerous, Specie_Dang_Lvl, Specie_Bio from Species natural join Biomes_Species natural join Biomes natural join Types where Type_id=1 order by Specie_id";
            $result = $connection->query($sql);
        }
