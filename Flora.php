...
<head>
    ...
    <ul class="nav-links">
        ...
        <li><a class="active" href="Flora.php">Flora</a></li>
...
<form action="Flora.php" method="post">
    ...
    <div>Search: <input type="text" name="search" placeholder="Ex: Pyrocoral, 2, etc ..."></div>
    <?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (!empty($type)) {
            ...
            if (!empty($search) || $search == 0) {
                if ($type == "Biome") {
                    $sql = "select Specie_id, Type_id, Specie_Name, Biome_Name, Type_Name, Specie_Dangerous, Specie_Dang_Lvl, Specie_Bio from Species natural join Types natural join Biomes_Species natural join Biomes where Type_id=5 and Biome_id='$search' order by Specie_id";
                }else if ($type == "Specie_id" || $type == "Type_id" || $type == "Specie_Dangerous" || $type == "Specie_Dang_Lvl") {
                    $sql = "Select Specie_id, Type_id, Specie_Name, Biome_Name, Type_Name, Specie_Dangerous, Specie_Dang_Lvl, Specie_Bio from Species natural join Types natural join Biomes_Species natural join Biomes where Type_id=5 and " . $type . " = $search order by Specie_id";
                }else if ($type == "Specie_Name" || $type ==  "Specie_Bio") {
                    $sql = "Select Specie_id, Type_id, Specie_Name, Biome_Name, Type_Name, Specie_Dangerous, Specie_Dang_Lvl, Specie_Bio from Species natural join Types natural join Biomes_Species natural join Biomes where Type_id=5 and " . $type . " like '%$search%' order by Specie_id";
                }else {
                ...
                if ($num_results == 0) {
                    //echo "No Results Found";
                    header('Refresh:1;URL=Flora.php');
                }
            }
            else if (!empty($edit)) {
                if ($type == "Biome") {
                ...
                $sql = "insert into Species(Specie_Name, Type_id, Specie_Dangerous, Specie_Dang_Lvl, Specie_Bio)values('$add_name', 5,$add_dangerous,$add_dang_lvl,'$add_bio')";
                $result = $connection->query($sql);
...
<?php
        ...
        if (empty($type) && empty($search) && empty($edit) && empty($add_name) && empty($delete)) {
            $sql = "Select Specie_id, Type_Name, Specie_Name, Biome_Name, Specie_Dangerous, Specie_Dang_Lvl, Specie_Bio from Species natural join Biomes_Species natural join Biomes natural join Types where Type_id=5 order by Specie_id";
            $result = $connection->query($sql);
        }
